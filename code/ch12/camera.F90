...
contains
  ...
  function cameraConstructor(lookFrom, lookAt, vup, vfov, aspect) result(cam)
    implicit none
    ...
    cam%origin = lookFrom
    cam%lowerLeftCorner = cam%origin - halfWidth*u - halfHeight*v - w
    cam%horizontal = 2.0*halfWidth*u
    cam%vertical = 2.0*halfHeight*v
...
program main
  ...
  ! setup camera
  cam = camera(lookFrom = [-2.0,2.0,1.0], &
       lookAt = [0.0,0.0,-1.0], &
       vup = [0.0, 1.0, 0.0], vfov = 20.0, aspect = real(nx)/ny)
