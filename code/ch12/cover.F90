...
contains
  ...
  function rngHost() result(res)
    real :: res
    call random_number(res)
  end function rngHost
...
program main
  use rayTracing
  implicit none
  integer, parameter :: nx = 1200, ny = 800
  integer, parameter :: maxNSpheres = 501
  integer :: i, j
  type(rgb) :: fb(nx,ny)
  type(environs) :: env
  !@cuf attributes(managed) :: env
  ...
  ! setup environment
  env%nSpheres = maxNSpheres
  call environsAlloc(env)

  lambArr(1) = lambertian(rgb([0.5, 0.5, 0.5]))
  cPtr = c_hostDevLoc(lambArr(1))
  env%spheres(1) = sphere([0.0, -1000.0, 0.0], 1000.0, lambMat, cPtr)

  dielArr(1) = dielectric(1.5)
  cPtr = c_hostDevLoc(dielArr(1))
  env%spheres(2) = sphere([0.0, 1.0, 0.0], 1.0, dielMat, cPtr)
  
  lambArr(2) = lambertian(rgb([0.4, 0.2, 0.1]))
  cPtr = c_hostDevLoc(lambArr(2))
  env%spheres(3) = sphere([-4.0, 1.0, 0.0], 1.0, lambMat, cPtr)
  
  metalArr(1) = metal(rgb([0.7, 0.6, 0.5]), 0.0)
  cPtr = c_hostDevLoc(metalArr(1))
  env%spheres(4) = sphere([4.0, 1.0, 0.0], 1.0, metalMat, cPtr)
  ...
  do ia = -11, 10
     do ib = -11, 10
        chooseMat = rngHost()
        center = [ia+0.9*rngHost(), 0.2, ib+0.9*rngHost()]        
        if ( sqrt(sum((center-[4.0, 0.2, 0.0])**2)) > 0.9 ) then
           if (chooseMat < 0.8) then ! diffuse
              i = i+1
              iLamb = iLamb+1
              lambArr(iLamb) = lambertian(rgb([ &
                   rngHost()*rngHost(), &
                   rngHost()*rngHost(), &
                   rngHost()*rngHost()]))
              cPtr = c_hostDevLoc(lambArr(iLamb))
              env%spheres(i) = sphere(center, 0.2, lambMat, cPtr)
           else if (chooseMat < 0.95) then !metal
              i = i+1
              iMetal = iMetal+1
              metalArr(iMetal) = metal(rgb([ &
                   0.5*(1.0+rngHost()), &
                   0.5*(1.0+rngHost()), &
                   0.5*(1.0+rngHost())]), 0.0)
              cPtr = c_hostDevLoc(metalArr(iMetal))
              env%spheres(i) = sphere(center, 0.2, metalMat, cPtr)
           else ! dielectric
              i = i+1
              iDiel = iDiel+1
              dielArr(iDiel) = dielectric(1.5)
              cPtr = c_hostDevLoc(dielArr(iDiel))
              env%spheres(i) = sphere(center, 0.2, dielMat, cPtr)
           end if
        end if
     end do
  end do

  ! even though 501 were allocated, not all iterations in the above
  ! resulted in adding a sphere, so update number of spheres
  env%nSpheres = i
